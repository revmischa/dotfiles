...
_gwt_dirs() {
  ...
  local dirs=("$base"/*(N/))
  [ ${#dirs} -eq 0 ] && { printf "\033[2m  no worktrees\033[0m\n"; return 1; }
  echo "${dirs[@]}"
}
...
gwf() {
  [ -z "$1" ] && { printf "\033[2m  usage: gwf <name>\033[0m\n"; return 1; }
  _gwt_session "$1" ""
}

gwfc() {
  [ -z "$1" ] && { printf "\033[2m  usage: gwfc <name>\033[0m\n"; return 1; }
  _gwt_session "$1" "--chrome"
}

gws() {
  local sessions=$(tmux list-sessions -F "#{session_name}" 2>/dev/null | grep '/')
  [ -z "$sessions" ] && { printf "\033[2m  no worktree sessions\033[0m\n"; return 1; }
  local choice
  if [ -n "$1" ]; then
    choice=$(echo "$sessions" | grep -F "$1" | head -1)
    [ -z "$choice" ] && { printf "\033[31m  no session matching '%s'\033[0m\n" "$1"; return 1; }
  elif command -v fzf >/dev/null; then
    choice=$(echo "$sessions" | fzf --prompt="  " --height=~40% --reverse)
    [ -z "$choice" ] && return 0
  else
    echo "$sessions"; printf "\033[2m  usage: gws <name>\033[0m\n"; return 1
  fi
  if [ -n "$TMUX" ]; then
...
gwcd() {
  local base=$(_gwt_base) || return 1
  if [ -n "$1" ]; then
    [ -d "$base/$1" ] || { printf "\033[31m  worktree '%s' not found\033[0m\n" "$1"; return 1; }
    cd "$base/$1"
  elif command -v fzf >/dev/null; then
